...
int Schema::getFieldsLength() {
    jmethodID buildID = jniEnv->GetMethodID(schemaClass, "getFieldsLength",
        "()I");
    if (buildID == NULL) {
        throw std::runtime_error("Can't find the method in java: getFieldsLength");
    }
    return jniEnv->CallIntMethod(schema, buildID);
};
...
char *Schema::getFieldName(int ordinal) {
    ...
    jmethodID buildID = jniEnv->GetMethodID(schemaClass, "getFieldName",
        "(I)Ljava/lang/String;");
    if (buildID == NULL) {
    ...
    jvalue args[1];
    args[0].i = ordinal;
    jobject fieldName = jniEnv->CallObjectMethodA(schema, buildID, args);
    return (char *) jniEnv->GetStringUTFChars((jstring) fieldName, JNI_FALSE);
};

char *Schema::getFieldDataTypeName(int ordinal) {
    checkOrdinal(ordinal);
    jmethodID buildID = jniEnv->GetMethodID(schemaClass, "getFieldDataTypeName",
        "(I)Ljava/lang/String;");
    if (buildID == NULL) {
...
char *Schema::getArrayElementTypeName(int ordinal) {
    checkOrdinal(ordinal);
    jmethodID buildID = jniEnv->GetMethodID(schemaClass, "getArrayElementTypeName",
        "(I)Ljava/lang/String;");
    if (buildID == NULL) {
