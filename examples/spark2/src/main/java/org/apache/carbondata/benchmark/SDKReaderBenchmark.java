...
public class SDKReaderBenchmark {
  public static void main(String[] args) throws InterruptedException, InvalidLoadOptionException, IOException {
    ...
    readNextCarbonRow(path, num, conf, 100000);
  }

  public static void readNextCarbonRow(String path, double num, Configuration conf, int printNum) {
    System.out.println("readNextCarbonRow");
    try {
      // Read data
      ...
      while (reader.hasNext()) {
        Object[] data = reader.readNextCarbonRow();
        i++;
        if (i % printNum == 0) {
  ...
  public static void readNextBatchRow(String path, double num, Configuration conf, int batch, int printNum) {
    ...
    try {
      ...
      while (reader.hasNext()) {
        ...
        Object[] batchRow = reader.readNextBatchRow(batch);
   ...
   */
  public static void writeData(String originPath, String newPath)
    ...
    for (int index = 0; index < i - 2; index++) {
      ...
      for (int k = 0; k < num; k++) {
        strings[0] = "from_email" + random.nextInt(ram) + row[0];
        strings[1] = "to_email" + random.nextInt(ram) + row[1];
